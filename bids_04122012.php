				...
				foreach($proxyPlusBids as $keyBids=>$rowPlaceBids){
						...
						if($amount == $rowPlaceBids['my_bid']){
							...
							$proxyplusstatus         = 0;
								...
								//Added on 04/12/2012 - To see if it solves the problem
								//$currentBid		     = $amount;
	...
	*/
	...
	if(!$database->loadObject($head_message)) {
		...
		if($bestBid->bid_price == $currentBid && $bestBid->id_offer == $id_offer && strcasecmp($bidType,"pp") == 0){
			...
			echo "proxyplusTOBID"."<pre>";
			print_r($proxyplusTOBID);
			echo "proxyplusTOBID"."</pre>";

		}else{
			echo "ELSE END16042011"."<br>";
		//END16042011
			$head_message->userid        = $currentuserid;
			$head_message->id_offer      = $id_offer;
			$head_message->bidtype       = $bidType;
