...
*/
	...
	function SendMails($userlist,$mailtype){
		...
		foreach($userlist as $can){
			...
			if($can->email) {
				 $this->writelog("\t\n----------------------\t\n--".$can->email."\t\n--".$subj."\t\n--".$mess);
				 $maillog = new mosMailsLog($this->_db);
				 $maillog->toaddr		= $can->email;
				 $maillog->subject		= $subj;
				 $maillog->message		= $mess;
				 $maillog->fromaddr		= $mosConfig_mailfrom;
				 $maillog->date_modified = date("Y-m-d H:i:s");
				 $maillog->store();
   ...
   function writelogdb($toemail, $subj, $mess, $mosConfig_mailfrom){
		$q = "INSERT INTO #__bid_auctions set closed_date=end_date, close_offer='1' where end_date<now() and id='$r->id' and (closed_date='0000-00-00' or closed_date='')";
		$this->_db->setQuery($q);
		$this->_db->query();
   }
   ...
   static function logiclog($str){
	   	$fp = fopen("logic.log","a");
		fwrite($fp,$str."\n");
		fclose($fp);
   }
    ...
    function ChooseWinner(){
    	...
    	$query = "SELECT b.* FROM #__bids b
        	where b.id_offer = $this->id order by bid_price desc, modified asc limit 1
    	";
    ...
    function GetBestBid($auctionid=0)
    {
		if($auctionid){
			$query = "SELECT * from #__bids b where id_offer='$auctionid' order by bid_price desc, modified asc";
		}else{
			$query = "SELECT * from #__bids b where id_offer='$this->id' order by bid_price desc, modified asc";
		}
		$this->writelog($query);
...
class mosMailsLog extends mosDBTable {
	var $id					= null;
	var $toaddr				= null;
	var $subject			= null;
	var $message	        = null;
	...
	function mosMailsLog(&$db){
		$this->mosDBTable('#__mails_log','id',$db);
	}
...
function writelogBids($str){
	...
	{
		$k = $this->_tbl_key;
		$this->writelogBids(print_r($this));
		if( $this->$k)
		{
			$ret = $this->_db->updateObject( $this->_tbl, $this, $this->_tbl_key, $updateNulls );
		}
		...
		if($this->id) {
			$id = $this->id;
			$this->id = "";
			$retBidLog = $this->_db->insertObject("#__bids_log", $this, $this->_tbl_key );
			$this->id  = $id;
		}
...
class mosBidProxyPlusBids extends mosDBTable {
	...
	function setDelete($ticketid = 0) {
		$query = "delete from ".$this->_tbl." WHERE ticket_id = ".$ticketid;
        $this->_db->setQuery($query);
        $this->_db->query();
	}
}
