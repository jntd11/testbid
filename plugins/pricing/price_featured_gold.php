...
class price_featured_gold  extends generic_pricing
{
    ...
    function price_featured_gold(&$db)
    {
        ...
        $this->classdescription='Module for buying featured Auctions (Gold)';
    ...
    function checktask($task,$d)
    {
        ...
        if ($task=='set_featured' && $d['featured']=='gold'){
            ...
            if ($this->_db->LoadResult()>0){
                ...
                $d['return_url']=$mosConfig_live_site.'/index.php?option=com_bids&task=set_featured&featured=gold&id='.$auctionid;
                $this->ShowPurchaseDialog($d);
                return true;
            }
        }

        if ($task=='buy_featured' && $d['featured']=='gold'){
            $this->ShowPurchaseDialog($d);
            return true;
        }
