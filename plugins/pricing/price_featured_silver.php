...
class price_featured_silver extends generic_pricing
{
    var $featured_type='silver';

    function price_featured_silver(&$db)
    {
        $this->classname='price_featured_silver';
        $this->classdescription='Module for buying featured Auctions (silver)';
    ...
    function checktask($task,$d)
    {
        ...
        if ($task=='set_featured' && $d['featured']=='silver'){
            ...
            if ($this->_db->LoadResult()>0){
                ...
                $d['return_url']=$mosConfig_live_site.'/index.php?option=com_bids&task=set_featured&featured=silver&id='.$auctionid;
                $this->ShowPurchaseDialog($d);
                return true;
            }
        }

        if ($task=='buy_featured' && $d['featured']=='silver'){
            $this->ShowPurchaseDialog($d);
            return true;
        }
