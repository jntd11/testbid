...
global $id,$cb_fieldmap, $mainframe,$lang,$bid_acl,$mosConfig_absolute_path;
...
class lic{
	...
	const start   = '15-07-2012';
	const licence = '27-12-2013';

	function checkValidity(){
		if(strtotime(self::licence) < time() || strtotime(self::start)  > time()){
			return false;
		}
		return true;
	}
	...
	function checkExpiration(){
		if(strtotime(self::licence) < (time() + (5*24*60*60))){
			return false;
		}
		return true;
	}
...
if($user->usertype != '' && $user->usertype != 'Registered' && !lic::checkValidity()){
	echo "<script>";
	echo "alert('Your Licence Expired');";
	echo "window.location = 'index.php';";
	echo "</script>";
	exit;
}elseif($user->usertype != '' && $user->usertype != 'Registered' && !lic::checkExpiration()) {
	echo "<script>";
	echo "alert('Your Licence Expires Soon.');";
	echo "</script>";
}
	
$id = intval( mosGetParam( $_REQUEST ,'id', 0 ) ); //bid id
$pageId = intval( mosGetParam( $_REQUEST ,'p', 0 ) ); //page Id

$bid_acl=new mosBidACL();

if (!$bid_acl->acl_check($task)) {
	return; //various ACL checks
}
...
switch($task) {
	...
	case 'checksession':
		 ...
		 foreach($arrSess as $key => $val) {
			  ...
			  if(!empty($val)) {
				$count++;
				$_SESSION['proxy'][$count] = explode("|",$val);
			  }
		 }
		$bid_acl=new mosBidACL();
		echo "1";
		if (!$bid_acl->acl_check("savebid")) {
			echo "0";
		}
		exit;
		break;
	case 'clearsession':
		...
		break;
	case 'viewbids':
		AuctionDetails( $option,$id, $pageId);//
		break;
	case 'mybidspicture':
	...
	case 'sendbid':
		...
		//Added for end date validation
		if($manaInfo->cb_buyerschoice == "Buyer's Choice"){
			if(strtotime($manaInfo->cb_enddate) < (time()+5)){
				mosRedirect($redirect_link, bid_expired_NA);
				exit;
			}
		}
		...
		*/
		...
		if($proxy && $id) {
			$oldBid = 0;
			$query = "SELECT a.my_bid FROM jos_proxyplus_bids a JOIN jos_proxyplus_tickets b ON (a.ticket_id = b.id) WHERE a.auction_id = ".$id." AND b.userid = ".$my->id." AND a.outbid = 0";
			$database->setQuery($query);
			$database->query();
			$oldBid = $database->loadResult();
			if($oldBid) {
				$redirect_link = mosRedirect($mosConfig_live_site."/index.php?option=$option&task=listauctionspicture&mosmsg=bidexists");
				exit;
			}

			$redirect_link = mosRedirect($mosConfig_live_site."/index.php?option=$option&task=editproxyticket&p=$id&v=$amount");
		}else{
			$retArray = saveBid($option,$id,$task);
			/*
			...
			$bestBid2=null;
			$bestBid2=$auction->GetBestBid($retArray['orgid_offer']);
			$bidType =	$retArray['bidType'];
			$myid	 =	$retArray['myid'];
			$aidorg  =  $retArray['aidorg'];
			$amount  =  $retArray['amount'];
			...
			$auction->writelog("$bidType $bestBid2->userid $myid");
			//echo "<b>$bidType $bestBid2->userid $orgmy->id</b>";

			if($bidType == "pp" && $bestBid2->userid != $myid) {
				/*
				 * TO STORE IN LOGS
				 */
				 $query = "INSERT INTO  #__bids_log (bid_price,id_proxy,modified,userid,id_offer,bidtype) VALUES (".$amount.", 0, '".date("Y-m-d H:i:s",time())."', ".$myid.",".$aidorg.",'m')";
				 $database->setQuery($query);
				 $database->query();
				
				$redirect_link=$mosConfig_live_site."/index.php?option=com_bids&task=viewbids&p=".$pageId."&orgTask=".$task."&id=$aidorg&Itemid=$Itemid&ou=$bidType&mosmsg=".bid_succes_special_new."#bid_list";
			}else {
				$redirect_link=$mosConfig_live_site."/index.php?option=com_bids&task=viewbids&p=".$pageId."&orgTask=".$task."&id=$aidorg&Itemid=$Itemid&mosmsg=".bid_succes_special."#bid_list";
			}
			//echo $redirect_link;
		}
		//echo $proxy;
		...
		*/
		if(strtotime($start_date) <= time() || $test) {
			...
			*/
			...
			$countAuctions = count($rowsMain);
			$auction = new mosBidOffers($database);
			//echo "<pre>";
			//print_r($rowsMain);
			do{
			 $loopAgain = 0;
		  	 foreach($rowsMain as $keyrowMain=>$rowsMainId) { 
				/*
				...
				if(!$auction->load($rowsMainId->id)){
					...
					//echo "rowsProxyPlusTickets";
					//print_r($rowsProxyPlusTickets);
					$proxyPlusBids = array();
					if(count($rowsProxyPlusTickets) > 0) {
						$countpriorityBids=0;
						foreach($rowsProxyPlusTickets as $rowProxyPlusTickets) {
							$query   = "SELECT * FROM #__proxyplus_bids AS b WHERE ticket_id = ".$rowProxyPlusTickets->id ." AND outbid = 0 ORDER BY priority, datemodified ASC LIMIT ".$rowProxyPlusTickets->lot_desired;
							$database->setQuery( $query);
							$rowsProxyPlusbids = $database->loadObjectList();
							//print_r($rowsProxyPlusbids);
							foreach($rowsProxyPlusbids as $rowProxyPlusbids) {
								//echo "<br>rowProxyPlusbids->auction_id == rowsMainId->id $rowProxyPlusbids->auction_id, $rowsMainId->id";
					...
					*  currentuserid:		 Highest bidder for particyular auction
								...
								if($amount == $rowPlaceBids['my_bid']){
									...
									if(!$currentHighestEqual){
										...
										*/
										...
										$query1 = "select * from #__bids where cancel=0 and userid='".$rowPlaceBids['userid']."' and id_offer='".$id_offer."' ";
										$database->setQuery($query1);
										$checkBidExists=new mosBids($database);//new head message

										if(!$database->loadObject($checkBidExists)) { 
											/*
											*/
											/*
											 * TO STORE IN LOGS
											 */
											 $query = "INSERT INTO  #__bids_log (bid_price,id_proxy,modified,userid,id_offer,bidtype) VALUES (".$rowPlaceBids['my_bid'].", 0, '".date("Y-m-d H:i:s",time())."', ".$rowPlaceBids['userid'].",".$id_offer.",'".$bidType."')";
											 $database->setQuery($query);
											 $database->query();
											 ...
											 * TO STORE IN Bids
											 ...
											 $query = "INSERT INTO  #__bids (bid_price,id_proxy,modified,userid,id_offer,bidtype) VALUES (".$rowPlaceBids['my_bid'].", 0, '".date("Y-m-d H:i:s",time())."', ".$rowPlaceBids['userid'].",".$id_offer.",'".$bidType."')";
											 $database->setQuery($query);
											 $database->query();
											 */
										}else{
											 $query = "UPDATE #__bids SET bid_price = ".$rowPlaceBids['my_bid'].", modified = '".date("Y-m-d H:i:s",time())."',  bidtype = $bidType WHERE  userid = ".$rowPlaceBids['userid']." AND id_offer = ".$id_offer;
											 $database->setQuery($query);
											 $database->query();
										}
			...
			$database->query();
			//echo "<b>keyrowMain".$keyrowMain."</b>";
...
function listAuctions( $option ) {
	...
	$limit 		  = intval( mosGetParam( $_REQUEST, 'l',  bid_opt_nr_items_per_page) );
	$limitstart   = intval( mosGetParam( $_REQUEST, 's', 0 ) );
	...
	$q = "select count(id) from #__bid_auctions a
	 	$where ";
	$database->setQuery($q);
	...
	/*$query =  "SELECT
				a.*, b.name,b.username, a.userid as userid, '$' as currency_name 
	           FROM #__bid_auctions as a
			   LEFT JOIN #__users as b ON b.id=a.userid
	";*/
	$query =   "SELECT a.link_extern, a.id, a.title, a.initial_price, a.description, a.shortdescription, a.picture, a.custom_fld1, a.custom_fld2, a.custom_fld3, a.custom_fld4, a.custom_fld5, b.name,b.username, a.userid as userid, '$' as currency_name, a.close_offer as close_offer,
				IFNULL((select max(bid_price) from jos_bids bids where bids.id_offer = a.id),0) as bid_price , (select userid from jos_bids bids where bids.id_offer = a.id order by bid_price desc, modified asc limit 1) as biduserid
				FROM jos_bid_auctions as a
				LEFT JOIN jos_users as b ON b.id=a.userid";
...
function listMyBids($option){
	...
	$query="select Distinct b.newmessages as bid_newmessages,
         b.*, '$' as currency_name, u.name as name, u.username
        from #__bids_log a 
        join #__bid_auctions as b on a.id_offer=b.id 
		join #__users u on u.id=b.userid ";
		...
		foreach ($rows as $keyTemp=>$rowTemp) {
			...
			(
			 ...
			 if($rowsbid=$database->loadObjectList()) {
				$rows[$keyTemp]->bid_price = $rowsbid[0]->bid_price;
				$rows[$keyTemp]->current_bidder = $rowsbid[0]->userid;
				$rows[$keyTemp]->biduserid = $rowsbid[0]->userid;
				$rows[$keyTemp]->bid_next = $rowsbid[0]->bid_price + findBidIncrement($rowsbid[0]->bid_price);
			...
			//Added for Finding proxy bids
			$queryProxyPlusBids = "SELECT auction_id, max(my_bid) as proxyplus_price, bp.outbid as outbid FROM #__proxyplus_bids AS bp  JOIN #__proxyplus_tickets bpt ON (bp.ticket_id = bpt.id) WHERE auction_id = ".$rowTemp->id." AND bpt.userid = $my->id";
...
function listMyWonBids($option){
	...
	$query="SELECT a.userid as myid, a.id as parent_message,a.bid_price,a.modified as bid_date,
	...
	$query .= " ORDER BY b.title + 0, b.title ASC";
	...
	foreach($rows as $keyTemp=>$rowTemp) {
			...
			$rows[$keyTemp]->biduserid = $rows[$keyTemp]->myid;
	}
...
function AuctionDetails($option, $id, $pageId) {
	...
	$database->setQuery($queryProxyPlusBids);
	  ...
	  */
	  if($pageId) {
		  //@todo commented on 5/20/2012 changed to normal
		  $prevpageId = $pageId - 1;
		  //$prevpageId = $pageId;
		  $query    =   "SELECT a.id  FROM jos_bid_auctions as a WHERE published=1 ORDER by a.title + 1, a.title ASC";
		  if($taskOrg == "mybidspicture"){
			$query    =   "SELECT a.id  FROM jos_bid_auctions as a JOIN jos_bids_log as b ON b.id_offer = a.id WHERE published=1 AND b.userid = '$my->id' ORDER by a.title + 1, a.title ASC";
		  }elseif($taskOrg == "mywonbidspicture"){
			$query    =   "SELECT a.id  FROM jos_bid_auctions as a JOIN jos_bids as b ON b.id_offer = a.id WHERE published=1 AND b.userid = '$my->id' and b.accept = 1 ORDER by a.title + 1, a.title ASC";
		  }elseif($taskOrg == "mywatchlistpicture" || $taskOrg == "listmywbigboard"){
			  $query    =   "SELECT a.id  FROM jos_bid_auctions as a JOIN  jos_bid_watchlist as b ON b.auctionid = a.id WHERE published=1 AND b.userid = '$my->id'  ORDER by a.title + 1, a.title ASC";
		  }

		  
		  $database->setQuery( $query,$prevpageId, 1);
		  $previousPage = $database->loadResult();
	  }
		  //@todo commented on 5/20/2012 changed to normal
		  $nextpageId = $pageId + 1;
		  //$nextpageId = $pageId;
		  ...
		  $database->setQuery( $query,$nextpageId, 1);
		  //echo "===$nextpageId, 1";
		  $nextPage = $database->loadResult();
  ...
  if ($my->id!=$auction->userid && (!isset($_COOKIE['washit'])||$_COOKIE['washit']<time()))
  {
	...
	$query = "select a.*,u.name,u.username from #__bids_log a
...
$loopcount = $loopcount1 = 0;
	...
	if(strtotime($auction->start_date) < time()){
		...
		*/
		$query   .= " ORDER BY ppt.datemodified, ppt.userid ASC";
		$database->setQuery( $query);
		$rowsProxyPlusTickets = $database->loadObjectList();
		$proxyPlusBids = array();
		echo "rowsProxyPlusTickets";
		...
		if(count($rowsProxyPlusTickets) > 0){
			$countpriorityBids=0;
			foreach($rowsProxyPlusTickets as $rowProxyPlusTickets) {
				$queryOutbid  = "SELECT id FROM #__proxyplus_bids AS b WHERE ticket_id = ".$rowProxyPlusTickets->id ." AND outbid = 0 and discontinue = 0 AND auction_id = ".$id_offer;
				$database->setQuery($queryOutbid);
				$outbidTODO = $database->loadResult();
				//echo "outbidarray";
				//print_R($outbidarray);
				$query   = "SELECT b.auction_id FROM #__proxyplus_bids AS b WHERE ticket_id = ".$rowProxyPlusTickets->id ." AND outbid = 0 AND discontinue = 1";
				$database->setQuery($query);
				$discontarray = $database->loadResultArray();
				$discontinueBid = 0;
				if(in_array($id_offer,$discontarray)){
					//echo "skandan1";
					$queryUpdate = "UPDATE #__proxyplus_bids SET discontinue = 0 WHERE auction_id = ".$id_offer." AND ticket_id = ".$rowProxyPlusTickets->id ;
					$database->setQuery($queryUpdate);
					$database->query();
					$discontinueBid = 1;

					$querylots = "SELECT * FROM #__proxyplus_bids ppb where ticket_id = ".$rowProxyPlusTickets->id." and outbid = 0 and is_revised = 1 ORDER BY priority limit 0, ".$rowProxyPlusTickets->lot_desired;
					$database->setQuery($querylots);
					$resultslots = $database->loadObjectList();
					$resultslots[0]->userid  = $rowProxyPlusTickets->userid;
					$proxyplusTOBID[]        = $resultslots[0];
					
					$queryupdate = "UPDATE #__proxyplus_bids ppb SET is_revised = 0 where id = ".$resultslots[0]->id;
					$database->setQuery($queryupdate);
					$database->query();
					
					$query   = "SELECT * FROM #__proxyplus_bids AS b WHERE ticket_id = ".$rowProxyPlusTickets->id ." AND outbid = 0 AND is_revised = 0 ORDER BY priority, datemodified ASC LIMIT ".$rowProxyPlusTickets->lot_desired;
				}elseif(count($discontarray) > 0){
					//echo "skandan2";
					$query   = "SELECT * FROM #__proxyplus_bids AS b WHERE ticket_id = ".$rowProxyPlusTickets->id ." AND outbid = 0 AND is_revised = 0 ORDER BY priority, datemodified ASC LIMIT ".$rowProxyPlusTickets->lot_desired;
				}else{
					//echo "skandan3";
				...
				$database->setQuery($query);
				$rowsProxyPlusbids = $database->loadObjectList();
				echo "<pre>"."rowsProxyPlusbids";
				print_r($rowsProxyPlusbids);
				echo "</pre>"."rowsProxyPlusbids";
				//print_r($rowsProxyPlusbids);

			
			$queryticket  = "SELECT count(b.id) FROM jos_proxyplus_tickets ppt  
						JOIN jos_proxyplus_bids b ON (ppt.id = b.ticket_id)  
						JOIN jos_bids bids ON (b.auction_id = bids.id_offer AND ppt.userid = bids.userid) 
						JOIN (SELECT * FROM `jos_bids` a  WHERE (id_offer, bid_price) in (select id_offer, max(bid_price) from `jos_bids` group by id_offer)) manual ON (bids.id = manual.id)
						 WHERE ppt.id = ".$rowProxyPlusTickets->id ." AND b.outbid = 0 AND b.is_revised = 0 AND b.discontinue = 0 ORDER BY b.priority";
			$database->setQuery($queryticket);
			$countActiveLots = $database->loadResult();
			echo '$countActiveLots'.$countActiveLots.'$rowProxyPlusTickets->lot_desired'.$rowProxyPlusTickets->lot_desired;
			if($countActiveLots < $rowProxyPlusTickets->lot_desired){
			
				foreach($rowsProxyPlusbids as $rowProxyPlusbids) {
					if(($rowProxyPlusbids->auction_id == $id_offer && $rowProxyPlusbids->discontinue == 0)) {
						$proxyPlusBids[strtotime($rowProxyPlusbids->datemodified)] = array(
														"id"=>$rowProxyPlusbids->id,
														"ticket_id"=>$rowProxyPlusbids->ticket_id,
														"lot_desired"=>$rowProxyPlusTickets->lot_desired,
					...
					//Start 12/18/2011 - Added for Outbidding 
					if($rowProxyPlusbids->id == $outbidTODO) {
							$outbidTODO = 0;
					}
					//END 12/18/2011
					/* Mayn't be needed to test
					if($rowProxyPlusbids->auction_id == $id_offer && $rowProxyPlusbids->discontinue == 1) {
						$queryUpdate = "UPDATE #__proxyplus_bids SET discontinue = 0 WHERE id = ".$rowProxyPlusbids->id;
						$database->setQuery($queryUpdate);
						$database->query();
					}
					*/
				...
				//12/18/2011 - added to outbidding the exisiting non pripority items - can add price also
				
				if($outbidTODO){
						//echo "<br>OUTBIDDING ";
						//1/6/2012   - Added Price too
						$queryPrice = "SELECT my_bid FROM #__proxyplus_bids WHERE auction_id = ".$id_offer." AND id = ".$outbidTODO ." LIMIT 1";
						$database->setQuery($queryPrice);
						$outbidPrice = $database->loadResult();
						//echo '$amount'.$amount;
						if($outbidPrice < $amount) { 
							$queryOutbit1 = "UPDATE #__proxyplus_bids SET outbid = 1 WHERE auction_id = ".$id_offer." AND id = ".$outbidTODO ." LIMIT 1";
							$database->setQuery($queryOutbit1);
							$database->query();
						}
				}
			}
		...
		*/
		$currentHighestEqual = 0;
		$currentProxyBid = array();
		do {
				$loopcount1 = 0;
				foreach($proxyPlusBids as $keyBids=>$rowPlaceBids){
						//echo "<br> currentuserid = ".$currentuserid;
						//echo "<br> amount = ".$amount;
						//echo "<br> currentBid =".$currentBid;
						//print_r($rowPlaceBids);
						$loopcount1++;
						// StartX - 9/13/2011 - added for first bid
						$queryBidNext = "SELECT if(max(b.bid_price)!='',max(b.bid_price),0) as bid_next, a.initial_price as initial_price FROM jos_bids AS b RIGHT JOIN jos_bid_auctions AS a ON a.id = b.id_offer WHERE a.id = ".$id_offer." ORDER BY bid_price DESC";
						$database->setQuery($queryBidNext);
						$amountProxyPlus = $database->loadObjectList();
						$currentProxyBid[$rowPlaceBids['userid']] = array(
																"price"=>$rowPlaceBids['my_bid'],
																"id"=>$rowPlaceBids['id'],
																"ticket_id"=>$rowPlaceBids['ticket_id'],
																"lot_desired"=>$rowPlaceBids['lot_desired']
																);
						echo '$rowPlaceBids'."<br>";
						print_R($rowPlaceBids);
						echo '$currentProxyBid'."<br>";
						print_R($currentProxyBid);
						echo '$currentuserid'.$currentuserid."<br>";
						//Same bidder
						//Added on 10-Apr-2011 for missing outbid on new proxy bid
						if($currentuserid == $rowPlaceBids['userid']) {
							//echo "HERE1";
							$proxyplusstatus++;
							if(count($proxyPlusBids) <= 1) {
								unset($proxyPlusBids[$keyBids]);
								//Added on 10-Apr-2011 - Added for first Bid
								//Start10042011
								//if($amount  > $rowPlaceBids['my_bid']){
									//$query1= "UPDATE #__proxyplus_bids SET outbid = 1 WHERE auction_id = $id_offer AND id = ".$rowPlaceBids['id'];
									//$database->setQuery($query1);
						...
						if($amount == $rowPlaceBids['my_bid']){
							...
							if(!$currentHighestEqual){
								...
								*/
								...
								//8/6/2011  Added for Missing 
								$queryrevised   = "SELECT * FROM #__proxyplus_bids AS b WHERE ticket_id = ".$rowProxyPlusTickets->id ." AND outbid = 0 AND is_revised = 1 ORDER BY priority, datemodified ASC LIMIT 1";
								$database->setQuery($queryrevised);
								$resultslotsrevised = $database->loadObjectList();
								
								if(count($resultslotsrevised) > 0 && $resultslotsrevised[0]->priority <= $resultslots[0]->priority){
									$resultslotsrevised[0]->userid = $rowPlaceBids['userid'];
									$proxyplusTOBID[] = $resultslotsrevised[0];

									$queryupdate = "UPDATE #__proxyplus_bids ppb SET is_revised = 0 where id = ".$resultslotsrevised[0]->id;
									$database->setQuery($queryupdate);
									$database->query();
							...
							$proxyplusstatus         = 0;
								...
								// FIx #16
								if($currentuserid  != $rowPlaceBids['userid']) {
									$queryOutbidHere3 = "UPDATE #__proxyplus_bids SET outbid = 1 WHERE auction_id = $id_offer AND my_bid < ".$rowPlaceBids['my_bid']. " AND outbid = 0 ";
									$database->setQuery($queryOutbidHere3);
									$database->query();
								}
								//END 1/15/2012
								$currentuserid       = $rowPlaceBids['userid'];
								$currentBid	         = $amount + findBidIncrement($amount);
								$amount		         = $rowPlaceBids['my_bid'];
								//StartX - 9/13/2011 - Added for Intial Price - not to increment at first time
								if($amountProxyPlus[0]->bid_next == 0) {
									$currentBid		     = $amountProxyPlus[0]->initial_price;
								}
								$bidType             = "pp";
								$currentHighestEqual = 1;
						}elseif($amount < $rowPlaceBids['my_bid'] && $amount + findBidIncrement($amount) > $rowPlaceBids['my_bid']){
							...
							//TODO Jai - Change to amount previosly it was $rowPlaceBids['my_bid']
							//3/31/2010
							//6/6/2010 Added = to $rowPlaceBids['my_bid'] > $currentBid
							if($rowPlaceBids['my_bid'] >= $currentBid && ($rowPlaceBids['my_bid'] + findBidIncrement($rowPlaceBids['my_bid']) < $amount)) {
								$currentBid	   = $rowPlaceBids['my_bid'] + findBidIncrement($rowPlaceBids['my_bid']);
							}elseif($rowPlaceBids['my_bid'] >= $currentBid){
								//Added on 3/25/2010 TODO Fix for #153
								$currentBid	   = $amount;
							}
							//Added on 4/12/2011 - Fix for start10042011
							if($rowPlaceBids['my_bid'] < $currentBid && (count($proxyPlusBids) == 0) && $currentuserid == $rowPlaceBids['userid']) {
								$currentBid	   = $rowPlaceBids['my_bid'];
							}
							$querylots = "SELECT * FROM `#__proxyplus_bids` ppb where ticket_id = ".$rowPlaceBids['ticket_id']." and outbid = 0 ORDER BY priority limit ".($rowPlaceBids['lot_desired']-1).", 1";
							$database->setQuery($querylots);
							...
							//8/6/2011 _ Added for missing condition
							$queryrevised   = "SELECT * FROM #__proxyplus_bids AS b WHERE ticket_id = ".$rowProxyPlusTickets->id ." AND outbid = 0 AND is_revised = 1 ORDER BY priority, datemodified ASC LIMIT 1";
							$database->setQuery($queryrevised);
							$resultslotsrevised = $database->loadObjectList();
							//echo "resultslotsrevised";
							//print_R($resultslotsrevised);
							if(count($resultslotsrevised) > 0 && $resultslotsrevised[0]->priority <= $resultslots[0]->priority){
								//echo "REVISED";
								$resultslotsrevised[0]->userid = $rowPlaceBids['userid'];
								$proxyplusTOBID[] = $resultslotsrevised[0];
							...
							// ENd 8/6/2011
							//print_r($proxyplusTOBID);
							$proxyplusstatus = 0;
							continue;
						}
				}
				echo count($proxyPlusBids);
...
*/
	...
	*/
	 ...
	 * Bidding Process START
	 */
	//echo "<br>"."isProxyPlus=".$isProxyPlus."countOrg=".$countOrg."auction_type=".$auction->auction_type."id_offer=".$id_offer;
	...
	//echo "<br>"."currentuserid".$currentuserid."my->id".$my->id."isProxyPlus".$isProxyPlus;
	if($currentuserid == $bestBid->userid && $bestBid->userid == $my->id && $isProxyPlus) {
		//echo "INSIDE".$currentuserid."HERE".$bestBid->userid;
		//exit;
	  //TODO
	...
	if(!$database->loadObject($head_message)) {
		...
		//Added on 4/14/2011 
		//start14042011 - done to decrement if it crosses the max limit Of proxy
		//echo "currentProxyBid";
		//print_r($currentProxyBid);
		if(isset($currentProxyBid[$currentuserid]["price"]) && $currentProxyBid[$currentuserid]["price"] < $currentBid) {
			$currentBid = $currentProxyBid[$currentuserid]["price"];
			$queryOutbid = "UPDATE #__proxyplus_bids SET outbid = 1 WHERE auction_id = $id_offer AND id = ".$currentProxyBid[$currentuserid]['id'];
			$database->setQuery($queryOutbid);
			$database->query();
			$arrOutbid[] = $currentuserid;

			$querylots = "SELECT * FROM `#__proxyplus_bids` ppb where ticket_id = ".$currentProxyBid[$currentuserid]['ticket_id']." and outbid = 0 ORDER BY priority limit ".($currentProxyBid[$currentuserid]['lot_desired']-1).", 1";
			$database->setQuery($querylots);
			$resultslots = $database->loadObjectList();
			$proxyplusstatus = 0;
			...
			if(count($resultslotsrevised) > 0 && $resultslotsrevised[0]->priority <= $resultslots[0]->priority){
				$resultslotsrevised[0]->userid = $currentuserid;
				$proxyplusTOBID[] = $resultslotsrevised[0];
				...
				$resultslots[0]->userid = $currentuserid;
				$proxyplusTOBID[] = $resultslots[0];
			}
		...
		//end14042011
		//Added on 4/16/2011 START16042011
		if($bestBid->bid_price == $currentBid && $bestBid->id_offer == $id_offer && strcasecmp($bidType,"pp") == 0){
			$queryOutbid = "UPDATE #__proxyplus_bids SET outbid = 1 WHERE auction_id = $id_offer AND id = ".$currentProxyBid[$currentuserid]['id'];
			$database->setQuery($queryOutbid);
			$database->query();
			$arrOutbid[] = $currentuserid;
		...
		//END16042011
			...
			}elseif($currentuserid != $bestBid->userid) { 
				$userMail	=	new mosUser($database);
				$userMail->load($bestBid->userid);
				$auction->SendMails(array($userMail),'bid_outbid');
			}elseif(!$isProxyPlus && ($orgmy->id != $bestBid->userid)) {
					/*
					 * To Test on 
					 */
					$userMail	=	new mosUser($database);
					$userMail->load($orgmy->id);
					$auction->SendMails(array($userMail),'bid_outbid');
			}
		//START16042011
		}
		...
		//echo "currentProxyBid";
		//print_r($currentProxyBid);
		//echo $currentProxyBid[$currentuserid]["price"] ."<". $currentBid;
	...
	*/
	/*
     * To maintain Return values only for first time
	 * before calling save bid again
	 * 
	 */
	if(!isset($retArr) || count($retArr) <= 0) {
		$retArr = array(
			...
			"bidType"	  =>  $bidType,
			"myid"		  =>  $orgmy->id,
			"orgid_offer" =>  $orgid_offer,
			"amount"	  =>  $orgamount
		);
	}

	//New Option - StartX - 9/3/2011 - to add bids from other tickets
	if($bidType == "m") {	
		$querytickets = "SELECT ppt.id as ticket_id, lot_desired FROM `#__proxyplus_bids` ppb JOIN `#__proxyplus_tickets` ppt ON (ppt.id = ppb.ticket_id) WHERE auction_id = ".$id_offer;
		$database->setQuery($querytickets);
		$resultstickets = $database->loadObjectList();
		echo $queryticket  = "SELECT count(b.id) FROM jos_proxyplus_tickets ppt  
				JOIN jos_proxyplus_bids b ON (ppt.id = b.ticket_id)  
				JOIN jos_bids bids ON (b.auction_id = bids.id_offer AND ppt.userid = bids.userid) 
				JOIN (SELECT * FROM `jos_bids` a  WHERE (id_offer, bid_price) in (select id_offer, max(bid_price) from `jos_bids` group by id_offer)) manual ON (bids.id = manual.id)
				 WHERE ppt.id = ".$resultstickets[0]->ticket_id ." AND b.outbid = 0 AND b.is_revised = 0 AND b.discontinue = 0 ORDER BY b.priority";
		$database->setQuery($queryticket);
		$countActiveLots = $database->loadResult();
		print_r($countActiveLots);

		
		//Startx - #18 - 9/25/2011 - bid to start from proxyplus
		$queryticket  = "SELECT count(b.id) FROM jos_proxyplus_tickets ppt  
				JOIN jos_proxyplus_bids b ON (ppt.id = b.ticket_id)  
				JOIN jos_bids bids ON (b.auction_id = bids.id_offer AND ppt.userid = bids.userid) 
				WHERE ppt.id = ".$resultstickets[0]->ticket_id ." AND b.outbid = 0 AND b.is_revised = 0 AND b.discontinue = 0 AND bids.bidType = 'pp' ORDER BY b.priority";
		$database->setQuery($queryticket);
		$countProxyActiveLots = $database->loadResult();
		print_r($countProxyActiveLots);
		//endx 
		
		/** Jai Commented on 11/12/2012 to test the below code

		$querylots = "SELECT * FROM `#__proxyplus_bids` ppb where ticket_id = ".$resultstickets[0]->ticket_id." and outbid = 0 ORDER BY priority limit ".($countProxyActiveLots).", 1";

		// Jai - added for userid - 09/12/2012
		echo $querylots = "SELECT ppt.userid as userid, b.id as id, b.ticket_id as ticket_id, b.auction_id as auction_id, b.my_bid as  my_bid, b.priority as priority, b.datemodified as datemodified, b.outbid as outbid, b.is_revised as is_revised, b.discontinue as discontinue FROM #__proxyplus_tickets ppt JOIN #__proxyplus_bids b ON (ppt.id = b.ticket_id)  WHERE b.ticket_id = ".$resultstickets[0]->ticket_id ." AND b.outbid = 0  ORDER BY b.priority LIMIT ".($countProxyActiveLots).", 1";
		$database->setQuery($querylots);
		$resultslots = $database->loadObjectList();
		print_r($resultslots);
		Comment end
		*/
		/** New code TEST 11/12/2012 - Start*/
		$queryManual = "SELECT distinct id_offer FROM `jos_bids` a WHERE (id_offer, bid_price) in (select id_offer, max(bid_price) from `jos_bids` group by id_offer) and userid = $my->id";
		$database->setQuery($queryManual);
		$manualActiveLots = $database->loadObjectList();
		if($countActiveLots < $resultstickets[0]->lot_desired) {
			//Startx - 9/14/2011 - Changed for incrementing Lot desired - Fix #25
			//added on $countActiveLots on limit
			echo $querylots  = "SELECT ppt.userid as userid, b.id as id, b.ticket_id as ticket_id, b.auction_id as auction_id, b.my_bid as  my_bid, b.priority as priority, b.datemodified as datemodified, b.outbid as outbid, b.is_revised as is_revised, b.discontinue as discontinue  FROM #__proxyplus_tickets ppt JOIN #__proxyplus_bids b ON (ppt.id = b.ticket_id)  WHERE ppt.id = ". $resultstickets[0]->ticket_id ." AND b.outbid = 0 AND auction_id not in ($queryManual) ORDER BY b.priority LIMIT ".($resultstickets[0]->lot_desired - $countActiveLots);
			$database->setQuery($querylots);
			$resultslots = $database->loadObjectList();
			echo '$resultslots';
			print_R($resultslots);
		}
		/** New code TEST 11/12/2012 - END */

		if(isset($resultslots[0]) && ($countActiveLots < $resultstickets[0]->lot_desired)){
			$proxyplusTOBID[] = $resultslots[0];
		}
		echo "IF";
		...
		//Added on 1/16/2012 - to add new Lots
		//#Fix for #16
		$querytickets = "SELECT ppt.id as ticket_id, lot_desired FROM `#__proxyplus_bids` ppb JOIN `#__proxyplus_tickets` ppt ON (ppt.id = ppb.ticket_id) WHERE auction_id = ".$id_offer;
		$database->setQuery($querytickets);
		$resultstickets = $database->loadObjectList();
	...
	*/
	foreach($proxyplusTOBID as $keyproxyplusTOBID=>$proxyplusTOBIDS){
		$queryCurrent = "SELECT if(max(b.bid_price)!='',max(b.bid_price),0) as bid_next, a.initial_price as initial_price FROM jos_bids AS b RIGHT JOIN jos_bid_auctions AS a ON a.id = b.id_offer WHERE a.id = ".$proxyplusTOBIDS->auction_id." ORDER BY bid_price DESC";
		$database->setQuery($queryCurrent);
		$amountProxyPlus = $database->loadObjectList();
		echo "amountProxyPlus";
		print_r($amountProxyPlus);
		echo '$proxyplusTOBIDS->my_bid'.$proxyplusTOBIDS->my_bid;
		echo '$amountProxyPlusNew'.$amountProxyPlusNew;
		echo '$amountProxyPlus[0]->bid_next'.$amountProxyPlus[0]->bid_next;
		if($amountProxyPlus[0]->bid_next) {
			$amountProxyPlusNew = $amountProxyPlus[0]->bid_next + findBidIncrement($amountProxyPlus[0]->bid_next);
			echo '$amountProxyPlusNew'.$amountProxyPlusNew;
			if(($proxyplusTOBIDS->my_bid < $amountProxyPlusNew) && ($proxyplusTOBIDS->my_bid >= $amountProxyPlus[0]->bid_next)) {
				echo "INSIDE";
				$amountProxyPlusNew = $proxyplusTOBIDS->my_bid;
			}
		}else{
			$amountProxyPlusNew = $amountProxyPlus[0]->initial_price;
		}
		echo "<br> INSIDE saveBids1 $option,$proxyplusTOBIDS->auction_id,$task,$amountProxyPlusNew, $proxyplusTOBIDS->ticket_id, $proxyplusTOBIDS->userid ";
		$auction->writelog("<br> INSIDE saveBids1 $option,$proxyplusTOBIDS->auction_id,$task,$amount, 0, $proxyplusTOBIDS->ticket_id, $proxyplusTOBIDS->userid ");
		mosBidOffers::logiclog("<br> INSIDE saveBids1 $option,$proxyplusTOBIDS->auction_id,$task,$amount, 0, $proxyplusTOBIDS->ticket_id, $proxyplusTOBIDS->userid ");
		//OLD No Ticket ids
		saveBid($option,$proxyplusTOBIDS->auction_id,$task,$amountProxyPlusNew, $proxyplusTOBIDS->ticket_id, $proxyplusTOBIDS->userid);
	}
	return $retArr;
...
function saveAuction($id,$option) {
	...
	//echo $p,$msg;
	mosRedirect( $mosConfig_live_site."/index.php?option=$option&task=viewbids&id=$auction->id&Itemid=$Itemid&p=".$p,$msg);
}
...
function saveProxyPlus($id,$option) {
	...
	$oldlotdesired = mosGetParam($_REQUEST,'oldlotdesired',0);
	$requestlotdesired = mosGetParam($_REQUEST,'lotdesired',0);
	$proxyTicket->lot_desired = $requestlotdesired;
	...
	if($countLots > 0) {
		...
		$isRunning = mosGetParam($_REQUEST,'run',0);
		if($proxyplustype != "new" && $isRunning == 0){
			$proxyPlusBid->setDelete($ticketidexisting);
		}
		$currentTicketId = ($currentTicketId)?$currentTicketId:$ticketidexisting;

		//set discontinue option	
		//echo $queryticket  = "SELECT count(b.id) FROM #__proxyplus_tickets ppt JOIN #__proxyplus_bids b ON (ppt.id = b.ticket_id)  WHERE ppt.id = ".$currentTicketId ." AND b.outbid = 0 AND b.is_revised = 0 AND b.discontinue = 0 ORDER BY b.priority";
		//Added on 7/23/2011 - to count the lots actually in active
		$queryticket  = "SELECT count(b.id) FROM #__proxyplus_tickets ppt JOIN #__proxyplus_bids b ON (ppt.id = b.ticket_id)  JOIN #__bids bids ON (b.auction_id = bids.id_offer AND ppt.userid = bids.userid) WHERE ppt.id = ".$currentTicketId ." AND b.outbid = 0 AND b.is_revised = 0 AND b.discontinue = 0 ORDER BY b.priority";

		//Added on 9/3/2011 - also includes Manual bids
		$queryticket  = "SELECT count(b.id) FROM jos_proxyplus_tickets ppt  
				JOIN jos_proxyplus_bids b ON (ppt.id = b.ticket_id)  
				JOIN jos_bids bids ON (b.auction_id = bids.id_offer AND ppt.userid = bids.userid) 
				JOIN (SELECT * FROM `jos_bids` a  WHERE (id_offer, bid_price) in (select id_offer, max(bid_price) from `jos_bids` group by id_offer)) manual ON (bids.id = manual.id)
				 WHERE ppt.id = ".$currentTicketId ." AND b.outbid = 0 AND b.is_revised = 0 AND b.discontinue = 0 ORDER BY b.priority";
		$database->setQuery($queryticket);
		$countActiveLots = $database->loadResult();
		//echo "<pre>";
		//echo "countActiveLots";

		
		for($i=1; $i <= 200;$i++) {
			$oldlotno = mosGetParam($_REQUEST,"oldlotno$i",0);
			$oldmybid = mosGetParam($_REQUEST,"oldmybid$i",0);
			$mybid    = mosGetParam($_REQUEST,"mybid$i",0);
			$is_outbid = mosGetParam($_REQUEST,"is_outbid$i",0); 
			$proxyPlusBid->id = NULL;
			$proxyPlusBid->ticket_id = ($currentTicketId)?$currentTicketId:$ticketidexisting;
			...
			$proxyPlusBid->my_bid		= $mybid;
			$proxyPlusBid->outbid       = 0;
			...
			*/
			...
			$query = "select count(a.id) as count, a.id as id, a.outbid from jos_proxyplus_bids a JOIN jos_proxyplus_tickets b WHERE a.ticket_id = b.id AND a.auction_id=$proxyPlusBid->auction_id AND b.userid = $my->id ";
			$database->setQuery($query);
			$rowtickets = $database->loadObjectList();
			//print_R($rowtickets);
			//print_r($proxyPlusBid);
			//echo $countActiveLots;
			//8/2/2011 - Changed >= to >
			if($countActiveLots > $proxyTicket->lot_desired) {
				$queryticket  = "SELECT b.priority as priority, b.id as id, ppt.lot_desired, b.auction_id as auction_id FROM #__proxyplus_tickets ppt JOIN #__proxyplus_bids b ON (ppt.id = b.ticket_id)  WHERE ppt.id = ".$ticketidexisting ." AND b.outbid = 0 AND b.is_revised = 0 AND b.discontinue = 0 ORDER BY b.priority LIMIT ".($proxyTicket->lot_desired - 1).", 1";
				//echo "<br>";
				$database->setQuery($queryticket);
				$lastticket = $database->loadObjectList();
				$auction = new mosBidOffers($database);
				$proxyPlusBid->auction_id;
				$bestBid = $auction->GetBestBid($lastticket[0]->auction_id);
				//print_R($bestBid);
				//$lastticket[0]->priority ." >= ".$proxyPlusBid->priority." && ".$bestBid->userid ." == ".$my->id;
				if($lastticket[0]->priority >= $proxyPlusBid->priority && $bestBid->userid == $my->id){
					$queryUpdate = "UPDATE #__proxyplus_bids SET discontinue = 1 WHERE id = ".$lastticket[0]->id;
					$database->setQuery($queryUpdate);
					$database->query();
					echo mysql_error();
				}
			}elseif($countActiveLots == $proxyTicket->lot_desired){ // 8/13/2011 - added new condition
				if(($oldmybid > 0 && $oldmybid < $mybid) && ($proxyPlusBid->priority <= $proxyTicket->lot_desired) && ($is_outbid == 1)) {
					//echo "8/13/2011 - condition ";
			...
			if($oldmybid > 0 && $oldmybid >= $mybid){
				continue;
			}
			$countickets = $rowtickets[0]->count;
		  	if($proxyPlusBid->auction_id != 0 && $countickets <= 0 || $proxyplustype == "new") {
				//echo "IF"."<br>";
				$stored = true;
				$proxyPlusBid->store();
			}else{
				...
				$query = "select count(a.id) as count, a.id as id, a.outbid from jos_proxyplus_bids a JOIN jos_proxyplus_tickets b WHERE a.ticket_id = b.id AND a.auction_id=$proxyPlusBid->auction_id AND b.userid = $my->id ";
				$database->setQuery($query);
				$rowsProxyIds = $database->loadObjectList();
				$proxyPlusBid->id = $rowsProxyIds[0]->id;
				$bestBidCurrent = null;
				$bestBidCurrent = $auction->GetBestBid($proxyPlusBid->auction_id);
				//print_r($bestBidCurrent);
				//echo "currentTicketId".$currentTicketId;
				//echo "<pre>";
				//print_r($proxyPlusBid);
				//echo "</pre>";
				//echo "$oldmybid  $bestBidCurrent->userid $my->id";
				if($oldmybid > 0 && $bestBidCurrent->userid != $my->id) {
				//	echo "TRUE";
					$proxyPlusBid->is_revised = 1;
				}
				else  {
					//echo "FALSE";
					$proxyPlusBid->is_revised = 0;
				}

				$proxyPlusBid->discontinue = 0;
				$proxyPlusBid->outbid      = 0;
				$stored = true;
				$proxyPlusBid->store();
			}
	...
	if($oldlotdesired != 0 && $oldlotdesired != $proxyTicket->lot_desired){
		$stored = true;
	}
	/*
	...
	*/
	...
	//echo $countActiveLots."<br>";
	//echo mysql_error();
	//print_r($proxyPlusBid);
	//echo "stored".(int)$stored;
	...
	if($stored && strtotime($start_date) <= time()){
		...
		//added on 9/3/2011 - Includes manual lot
		//@todo removed on 12/10/2011 "AND b.discontinue = 0" condition to fix #24
		$queryticket  = "SELECT count(b.id) FROM jos_proxyplus_tickets ppt  
				JOIN jos_proxyplus_bids b ON (ppt.id = b.ticket_id)  
				JOIN jos_bids bids ON (b.auction_id = bids.id_offer AND ppt.userid = bids.userid) 
				JOIN (SELECT * FROM `jos_bids` a  WHERE (id_offer, bid_price) in (select id_offer, max(bid_price) from `jos_bids` group by id_offer)) manual ON (bids.id = manual.id)
				 WHERE ppt.id = ".$proxyPlusBid->ticket_id ." AND b.outbid = 0 AND b.is_revised = 0 ORDER BY b.priority";
		$database->setQuery($queryticket);
		$countActiveLots = $database->loadResult();
		//echo "countActiveLots".$countActiveLots;
		$test = $database->loadObjectList();
		//echo "<pre>";
		//print_r($test);
		//exit;
		...
		if($countActiveLots < $proxyTicket->lot_desired) {
			//Startx - 9/14/2011 - Changed for incrementing Lot desired - Fix #25
			//added on $countActiveLots on limit
			$query  = "SELECT *, b.id as bid FROM #__proxyplus_tickets ppt JOIN #__proxyplus_bids b ON (ppt.id = b.ticket_id)  WHERE ppt.id = ".$proxyPlusBid->ticket_id ." AND b.outbid = 0 AND auction_id not in ($queryManual) ORDER BY b.priority LIMIT ".($proxyTicket->lot_desired - $countActiveLots);
			$database->setQuery( $query);
			$rowsProxyPlusbids = $database->loadObjectList();
		}
		//ENDED ENDX on 9/3/2011 - Includes manual lot

		//echo "HERE".count($rowsProxyPlusbids);
		//echo "HEREJai".$countActiveLots ."< ".$proxyTicket->lot_desired;
		//print_r($rowsProxyPlusbids);
		if(count($rowsProxyPlusbids) > 0 && $countActiveLots < $proxyTicket->lot_desired) {
			//echo "inside";
			foreach($rowsProxyPlusbids as $rowProxyPlusbids) {
				$proxyPlusBids[$rowProxyPlusTickets->userid] = array(
													...
													"datemodified"=>$rowProxyPlusbids->datemodified,
													"outbid"=>$rowProxyPlusbids->outbid,
													"revised"=>$rowProxyPlusbids->is_revised,
													"discontinue"=>$rowProxyPlusbids->discontinue,
													"userid"=>$rowProxyPlusTickets->userid
												  );

				$querycheck = "SELECT * FROM #__proxyplus_bids b WHERE b.ticket_id = ".$rowProxyPlusbids->ticket_id ." AND b.outbid = 0 AND b.is_revised = 0 ORDER BY b.priority";
				$database->setQuery($querycheck);
				$rowsCheck = $database->loadObjectList();
				//print_r($rowsCheck);
				//echo 'count($rowsCheck)'. count($rowsCheck)." < ".$lotdesired;
				if(count($rowsCheck) <= $lotdesired){
					$queryupdate = "UPDATE #__proxyplus_bids ppb SET is_revised = 0 where id = ".$rowProxyPlusbids->bid;
					$database->setQuery($queryupdate);
					$database->query();
				}
				...
				//echo "<br>br->".$option, $rowProxyPlusbids->auction_id, $task, $amount, $rowProxyPlusbids->ticket_id, $rowProxyPlusbids->userid;
...
function checkBB($option){
	...
	$query  = "SELECT count(a.id) FROM #__proxyplus_bids AS b JOIN #__bid_auctions AS a ON a.id = b.auction_id JOIN #__proxyplus_tickets pt  ON (pt.id = b.ticket_id) WHERE a.title = '".$lotid."' AND pt.userid = ".$my->id ." AND a.userid <> ".$my->id." AND pt.id <> ".$ticketid." AND outbid <> 1" ;
		$database->setQuery( $query);
		$countBids = $database->loadResult();
		if($countBids > 0) { 
			return 1;
		}
		$query  = "SELECT a.id as auction_id, b.id as id, id_offer,  b.userid, bid_price,  IF(b.bid_price > 0,b.bid_price + a.min_increase, initial_price)  as bid_next, a.shortdescription  FROM #__bids AS b RIGHT JOIN #__bid_auctions AS a ON a.id = b.id_offer WHERE a.userid <> ".$my->id." AND a.title = '".$lotid."' ORDER BY bid_price DESC";
	}
	$database->setQuery( $query);
	//echo $database->_sql;exit;
	$rows = $database->loadObjectList();
...
function checkManualLot() {
	global $database, $my, $mosConfig_absolute_path;
	global $mosConfig_live_site,$task,$mosConfig_offset;
	global $Itemid;
	
	$lotid   = intval( mosGetParam( $_REQUEST, 'lot', 0 ) );
	$type	 = intval( mosGetParam( $_REQUEST, 'type', 0 ) );

	$query = "SELECT id, id_offer, bid_price FROM #__bids WHERE id_offer = '".$lotid."' AND bidtype = 'M' AND userid = ".$my->id." ORDER BY bid_price DESC LIMIT 1";
	$database->setQuery( $query);
	$rows = $database->loadObjectList();
	return isset($rows[0]->bid_price)?$rows[0]->bid_price:0;
}
function listBigBoard( $option , $type=0){
	...
	$query =  "SELECT a.title as title, a.shortdescription as shortdescription,	a.id as id, a.userid, a.initial_price, a.currency, a.BIN_price, a.auction_type, a.start_date, a.end_date, a.published, a.close_offer, a.cat, a.winner_id, a.min_increase, a.reserve_price, b.username, c.name AS currency_name, cats.catname, cats.id AS cid, d.initial_bid, d.bid_price, d.userid  as bid_user, d.bid_price + a.min_increase as bid_next, d.modified as bid_modified, d.id as bid_id, d.bidtype as bidtype 
		FROM #__bid_auctions AS a
		LEFT JOIN #__users AS b ON b.id = a.userid
		LEFT JOIN #__bid_currency AS c ON c.id = a.currency
	...
	$query  .= "SELECT a.title as title, a.shortdescription as shortdescription, a.id  as id, a.userid, a.initial_price, a.currency, a.BIN_price, a.auction_type, a.start_date, a.end_date, a.published, a.close_offer, a.cat, a.winner_id, a.min_increase, a.reserve_price, b.username, c.name AS currency_name, cats.catname, cats.id AS cid, 0 as initial_bid, 0 as bid_price, 0 as bid_user, a.initial_price as bid_next,  0 as bid_modified, 0 as bid_id, '' as bidtype 
		FROM jos_bid_auctions AS a
		LEFT JOIN #__users AS b ON b.id = a.userid
		LEFT JOIN #__bid_currency AS c ON c.id = a.currency
		LEFT JOIN #__bid_categories AS cats ON a.cat = cats.id
...
function listProxyTicket( $option , $type=0) {
	...
	if($task != "editproxyticket") {
		echo '<script type="text/javascript">a = setInterval("refreshMe();",'.(bid_opt_refresh_minutes  * 1000 * 60).');</script>';
	}
	...
	foreach($rowstickets as $rowticket) {
		$query	 = "SELECT ptb.*, ba.title as title, ba.shortdescription as shortdesc, ba.start_date as start_date, ba.end_date as end_date, close_offer FROM  #__proxyplus_bids ptb JOIN #__bid_auctions ba ON ( ptb.auction_id = ba.id) ";
	...
	$sfilters['bid_type'] = $filter_bidtype;
	$sfilters['cat'] = $filter_category;
	//@TODO to check $pageNav - was $pageNav initially
	HTML_Auction::listProxyTicket( $rows, $lists, null,$sfilters,$lots_desired,$userticket);
}
//JaiStartI
