...
{* JaiEndB *}
				...
				<tr>
					...
					<td width="30%">
						<div id="auction_title">
							<a class="linkpicture" href="{if $task=='mybids'}{$current_row->links.bid_list}{else}{$current_row->links.auctiondetails}{/if}"><span id="auctitlelabel">{$smarty.const.auction_title}</span></a>
							<a class="linkpicture" href="{if $task=='mybids'}{$current_row->links.bid_list}{else}{$current_row->links.auctiondetails}{/if}">
							<span id="auctitle">{$current_row->title}</span>
							</a>
					...
					<td width="20%">
						<a class="linkpicture" href="{if $task=='mybids'}{$current_row->links.bid_list}{else}{$current_row->links.auctiondetails}{/if}"><span id="labelshortdesc">{$smarty.const.label_short_desc}</span></a>
						<a class="linkpicture" href="{if $task=='mybids'}{$current_row->links.bid_list}{else}{$current_row->links.auctiondetails}{/if}"><span id="shortdesc">{$current_row->shortdescription}</span></a>
					</td>
					<td width="25%">
					{* LDHEndE *}
	...
	{* JAIENDJ *}
					<td rowspan="4" valign="top">
						...
						{$current_row->currency_name}&nbsp;<input name="amount{$current_row->id}" autocomplete="off" id="amount{$current_row->id}" onkeypress="return onlyNumbers(event);" class="inputbox" type="text" value="" size="20" alt="bid" {$disable_bids}>&nbsp;
						</div>
						{/if}
						 {if  !$current_row->expired && $current_row->is_my_auction != 1}
							<div class="spacing-top">
