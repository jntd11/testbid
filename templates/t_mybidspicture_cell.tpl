				...
				<tr>
					...
					<td width="10%">
						<div id="auction_title">
							...
							<a href="{if $task=='mybids'}{$current_row->links.bid_list}{else}{$current_row->links.auctiondetails}{/if}">
	...
	{* JAIENDJ *}
					<td rowspan="4" valign="top">
						...
						{if $current_row->proxyplus_price != 0}
							 ...
							 <input type="hidden" name="maxproxyplus{$current_row->id}" id="maxproxyplus{$current_row->id}" value="{$current_row->proxyplus_price}">
					...
					{if $smarty.const.bid_allow == 1}
						...
						{$current_row->currency_name}&nbsp;<input name="amount{$current_row->id}" autocomplete="off" id="amount{$current_row->id}" class="inputbox" onkeypress="return onlyNumbers(event);"  type="text" value="" size="20" alt="bid" {$disable_bids}>&nbsp;
						{/if}
					{/if}
