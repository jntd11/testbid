...
{* JaiEndB *}
					{*  LDHEndA *}
		<td id="auction_thumb{$class}_old"  valign="top" width="20%">
			...
			<br>
			<input type="button" onclick="window.open('{$current_row->link_extern}','targetWindow', 'toolbar=no,location=no,status=no,menubar=no,scrollbars=no,resizable=no,width=720,height=480')" class="button art-button" id="refresh" value="{$smarty.const.external_link}">
			{/if}
		</td>
				...
				<tr>
					...
					<td width="10%">
						<div id="auction_title">
							...
							<a class="linkpicture" href="{if $task=='mybids'}{$current_row->links.bid_list}{else}{$current_row->links.auctiondetails}{/if}">
							<span id="auctitle{$current_row->id}">{$current_row->title}</span>
							</a>
	...
	{* JAIENDJ *}
					...
					{if $current_row->proxyplus_price != 0}
						 <div class="spacing-top"><input type="hidden" name="maxproxyplus{$current_row->id}" id="maxproxyplus{$current_row->id}" value="{$current_row->proxyplus_price}">
					...
					<div class="spacing-top"><input type="hidden" name="prxo{$current_row->id}" id="prxo{$current_row->id}" value="0">
					...
					{/if}
					 ...
					 {if $smarty.const.bid_allow == 1} 
						  {if  !$current_row->expired && $current_row->is_my_auction != 1}
							<div class="spacing-top"><span id="bid{$current_row->id}" >
						...
						{$current_row->currency_name}&nbsp;<input name="amount{$current_row->id}" id="amount{$current_row->id}" class="inputbox" type="text" value="" onkeypress="return onlyNumbers(event);"   size="20" alt="bid" {$disable_bids}>&nbsp;
						</div>
						{/if}
					 {/if}
					{if  !$current_row->expired && $current_row->is_my_auction != 1 && $smarty.const.bid_allow == 1}
						<div class="spacing-top"><input type="submit" name="send" value="{$smarty.const.but_send_bid}"  class="button art-button" {$disable_bids} /></div>
					{/if}
