...
class getmybidsTab extends cbTabHandler {
		...
		if($mybids) {
			...
			foreach ($mybids as $mb){
    			...
    			if ($mb->auction_type==AUCTION_TYPE_PUBLIC){
    			    //Outbid?
    			    $database->setQuery("select count(*) from #__bids where id<>'$mb->parent_message' and bid_price>$mb->bid_price");
    			    $outbid=$database->loadResult();
    			    if ($outbid) $msg=bid_outbidded;
    			    else $msg=bid_highest_bid;
    			}else{
    			    $msg=bid_private;
    			}
    			...
    			//var_dump($myratings);exit;
    			 ...
    			 $return .='</td>';
    			 $return .='<td>';
    			 $return .= date(bid_opt_date_format,strtotime($mb->end_date));
    			 $return .='</td>';
    			 $return .='<td>';
